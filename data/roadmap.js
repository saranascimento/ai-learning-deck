 ...
 * roadmap/04-software-design.md, roadmap/05-platform-engineering.md,
 * roadmap/06-architecture-system-design.md e roadmap/07-ai-engineering.md (Fase 2). Este
 ...
 *
 * Epics 01–07: Fase 2 concluída → navegáveis até Conceito.
 *
    ...
    epic({
      ...
      stories: [
        story({
          ...
          tasks: [
            task({ order: 10, title: "Architecture vs Design", note: "ponto de entrada — granularidade e horizonte de decisão" }),
            task({ order: 20, title: "Architectural Characteristics", note: "'-ilities': o que a arquitetura otimiza além de função" }),
            task({ order: 30, title: "Architectural Constraints", note: "o que não pode mudar (regulatório, org, legado)" }),
            task({
              order: 40,
              title: "Trade-offs",
        ...
        story({
          ...
          tasks: [
            task({ order: 10, title: "Layered Architecture", note: "ponto de entrada do catálogo de estilos" }),
            task({
              order: 20,
            ...
            task({ order: 30, title: "Monolith", note: "ponto de contraste com o resto do catálogo" }),
            task({ order: 40, title: "Modular Monolith", requires: ["Monolith", "Architecture Fundamentals / Modularity"], note: "aplica Modularity no nível de deployment unit" }),
            task({
              order: 50,
              title: "Microservices",
            ...
            task({ order: 60, title: "Service-Oriented Architecture (SOA)", collision: "frequentemente confundida com Microservices — ESB/governança pesada × independência de deploy" }),
            task({ order: 70, title: "Event-Driven Architecture", note: "framing conceitual; mecânica detalhada em Messaging (Story 10)" }),
            task({
              order: 80,
            ...
            task({ order: 90, title: "Microkernel Architecture", note: "plugin/plug-in architecture — nicho (IDEs, browsers); mantida como Task própria" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Functional Requirements", note: "ponto de entrada" }),
            task({ order: 20, title: "Non-Functional Requirements", requires: ["Functional Requirements"] }),
            task({
              order: 30,
            ...
            task({ order: 40, title: "Bottlenecks", note: "onde o sistema quebra sob carga", revisit: ["System Design Fundamentals / Capacity Estimation", "Architecture Fundamentals / Trade-offs"] }),
            task({ order: 50, title: "Single Point of Failure", note: "ponte para Availability & Reliability", revisit: ["System Design Fundamentals / Bottlenecks"] }),
          ],
        }),
        story({
          slug: "scalability",
          ...
          tasks: [
            task({ order: 10, title: "Scalability", collision: "≠ Performance (Platform / Performance Engineering) — 'rápido para 1' × 'rápido para N'" }),
            task({ order: 20, title: "Vertical Scaling", requires: ["Scalability"] }),
            task({ order: 30, title: "Horizontal Scaling", requires: ["Scalability"], collision: "contraste direto com Vertical Scaling" }),
            task({ order: 40, title: "Statelessness", requires: ["Horizontal Scaling"], note: "consolidação 2→1 (Stateless + Stateful Systems, contraste ensinado em par) — o que torna Horizontal Scaling tratável" }),
            task({
              order: 50,
            ...
            task({ order: 70, title: "Backpressure", note: "canônico do roadmap — controle de fluxo produtor/consumidor", revisit: ["Programming Foundations / Concurrency"] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Distributed Cache", requires: ["Platform / Caching / Cache", "Scalability / Horizontal Scaling"], note: "cache compartilhado entre réplicas — o que muda vs cache local" }),
            task({ order: 20, title: "Cache Stampede", requires: ["Distributed Cache", "Platform / Caching / Cache Invalidation"], note: "muitas requisições batem no DB simultaneamente após expiração" }),
            task({ order: 30, title: "Cache Coherence", requires: ["Distributed Cache"], note: "consistência entre múltiplas instâncias de cache" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Availability", subtopics: ["'nines' (99.9%…)", "High Availability (o alvo/prática)"] }),
            task({ order: 20, title: "Reliability", collision: "uptime (Availability) × corretude de comportamento ao longo do tempo" }),
            task({ order: 30, title: "Fault Tolerance" }),
            task({ order: 40, title: "Redundancy" }),
            task({ order: 50, title: "Failover", requires: ["Redundancy"], note: "pressupõe que existe um backup redundante para o qual chavear" }),
            task({ order: 60, title: "Graceful Degradation", requires: ["Fault Tolerance"], note: "estratégia específica dentro do guarda-chuva Fault Tolerance" }),
            task({
              order: 70,
        ...
        story({
          ...
          tasks: [
            task({ order: 10, title: "Distributed System", collision: "≠ Client-Server Model (Platform / Web Fundamentals) — teoria geral × topologia particular" }),
            task({ order: 20, title: "Partial Failure", requires: ["Distributed System"] }),
            task({ order: 30, title: "Network Partition", requires: ["Partial Failure"], note: "tipo específico de falha parcial" }),
            task({ order: 40, title: "Clock & Time in Distributed Systems", requires: ["Distributed System"], note: "clocks lógicos, NTP drift, ordenação sem clock global" }),
            task({
              order: 50,
            ...
            task({ order: 60, title: "CAP Theorem", requires: ["Consistency Models (Strong / Eventual)", "Network Partition"] }),
            task({ order: 70, title: "PACELC", requires: ["CAP Theorem"], note: "extensão/refinamento do CAP" }),
            task({ order: 80, title: "Consensus", requires: ["Network Partition"], revisit: ["Distributed Systems Fundamentals / Consistency Models (Strong / Eventual)"] }),
            task({ order: 90, title: "Leader Election", requires: ["Consensus"], note: "aplicação concreta do problema geral de consenso" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Replication", requires: ["Distributed Systems Fundamentals / Distributed System"], note: "REPOINT — antes Requires: Consistency Models; mecânica de replicação é definível sobre 'sistema distribuído', sem exigir o vocabulário de consistência" }),
            task({
              order: 20,
            ...
            task({ order: 30, title: "Partitioning", collision: "replicar copia, particionar divide — conceitos contrastantes", revisit: ["Data Distribution / Replication"] }),
            task({ order: 40, title: "Sharding", requires: ["Partitioning"], subtopics: ["estratégias: range, hash, directory"] }),
            task({ order: 50, title: "Consistent Hashing", requires: ["Sharding"] }),
            task({ order: 60, title: "Hotspot", requires: ["Sharding"], note: "realocado de Scalability — carga desigual entre partições/nós", revisit: ["Data Distribution / Consistent Hashing"] }),
            task({ order: 70, title: "Rebalancing", requires: ["Sharding"], note: "o que acontece ao adicionar/remover shards" }),
          ],
        }),
        story({
          ...
          tasks: [
            ...
            task({ order: 20, title: "RPC", revisit: ["Service Communication / Synchronous vs Asynchronous Communication"] }),
            task({ order: 30, title: "gRPC", requires: ["RPC"], note: "sub/exemplo: Protocol Buffers" }),
            task({ order: 40, title: "Service Discovery" }),
            task({
              order: 50,
              title: "API Gateway",
            ...
            task({ order: 60, title: "Reverse Proxy" }),
            task({ order: 70, title: "Service Mesh", requires: ["API Gateway", "Reverse Proxy", "Service Discovery"], note: "capstone — combina os anteriores em infraestrutura dedicada" }),
          ],
        }),
        story({
          slug: "messaging",
          ...
          tasks: [
            task({ order: 10, title: "Message Queue", subtopics: ["Producer & Consumer (papéis)"] }),
            task({ order: 20, title: "Message Broker", requires: ["Message Queue"], note: "infraestrutura que implementa a fila" }),
            task({ order: 30, title: "Pub/Sub", subtopics: ["Topic (unidade de endereçamento)"], revisit: ["Messaging / Message Queue"] }),
            task({
              order: 40,
              title: "Event",
            ...
            task({ order: 50, title: "Consumer Groups", requires: ["Message Broker"], note: "consumo paralelo/escala" }),
            task({
              order: 60,
            ...
            task({ order: 80, title: "Dead Letter Queue", requires: ["Delivery Semantics (At-Most-Once / At-Least-Once / Exactly-Once)"], note: "a válvula de escape do retry do at-least-once" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Timeout" }),
            task({ order: 20, title: "Retry", revisit: ["Resilience Patterns / Timeout"] }),
            task({
              order: 30,
              title: "Idempotency",
            ...
            task({ order: 40, title: "Backoff", requires: ["Retry"] }),
            task({ order: 50, title: "Jitter", requires: ["Backoff"] }),
            task({ order: 60, title: "Retry Storm", requires: ["Retry", "Backoff", "Jitter"], note: "a patologia do próprio cluster Retry/Backoff/Jitter" }),
            task({ order: 70, title: "Circuit Breaker", revisit: ["Resilience Patterns / Timeout", "Resilience Patterns / Retry"] }),
            task({ order: 80, title: "Bulkhead", revisit: ["Resilience Patterns / Circuit Breaker"] }),
            task({
              order: 90,
              title: "Rate Limiting",
        ...
        story({
          ...
          tasks: [
            task({ order: 10, title: "Distributed Transaction", note: "unidade de trabalho através de múltiplos serviços/bancos" }),
            task({ order: 20, title: "Two-Phase Commit (2PC)", requires: ["Distributed Transaction"], note: "solução clássica bloqueante" }),
            task({
              order: 30,
              title: "Saga Pattern",
            ...
            task({ order: 40, title: "Choreography", requires: ["Saga Pattern"] }),
            task({ order: 50, title: "Orchestration", requires: ["Saga Pattern"] }),
            task({
              order: 60,
        ...
        story({
          ...
          tasks: [
            ...
            task({ order: 20, title: "Event Sourcing", requires: ["Messaging / Event"], revisit: ["Data & Architecture Patterns / CQRS"] }),
            task({ order: 30, title: "Materialized View", revisit: ["Data & Architecture Patterns / CQRS"] }),
            task({ order: 40, title: "Database per Service", requires: ["Architectural Styles / Microservices"], subtopics: ["Shared Database (anti-padrão, contraste)"] }),
            task({
              order: 50,
        ...
        story({
          ...
          tasks: [
            ...
            task({ order: 20, title: "Fitness Functions", requires: ["Evolutionary Architecture"], revisit: ["Testing & Quality Engineering / Testing Strategy"] }),
            task({ order: 30, title: "Strangler Fig Pattern", requires: ["Evolutionary Architecture"], revisit: ["Architectural Styles / Monolith"] }),
            task({
              order: 40,
            ...
            task({ order: 50, title: "Build vs Buy", requires: ["Architecture Fundamentals / Trade-offs"], note: "decisão de trade-off aplicada" }),
          ],
        }),
      ],
    }),
    epic({
      ...
      stories: [
        story({
          ...
          tasks: [
            task({ order: 10, title: "Artificial Intelligence (AI)", note: "raiz — sistemas que exibem comportamento tipicamente associado à inteligência", resources: [{ type: "deck", deckId: "ai-fundamentals" }] }),
            task({ order: 20, title: "AI System", requires: ["Artificial Intelligence (AI)"], collision: "≠ AI Model — o sistema de engenharia completo (APIs, dados, guardrails) que usa 1+ modelos, não o modelo em si" }),
            task({ order: 30, title: "Machine Learning (ML)", requires: ["Artificial Intelligence (AI)"], subtopics: ["Deep Learning (ML baseado em redes neurais)"], note: "subcampo — aprender padrões a partir de dados" }),
            task({ order: 40, title: "AI Model", requires: ["Machine Learning (ML)"], subtopics: ["Model Architecture"], collision: "≠ AI System — o artefato (arquitetura + parâmetros) produzido pelo treino" }),
            task({ order: 50, title: "Training", requires: ["AI Model"], note: "processo que produz os parâmetros do modelo", resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 60, title: "Inference", note: "usar um modelo já treinado — contraste direto com Training; contraste/sequência, não prerequisite", revisit: ["AI Fundamentals / Training"], resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 70, title: "Parameters", requires: ["AI Model"], note: "os valores aprendidos durante o treino" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Language Model (LM)", requires: ["AI Fundamentals / AI Model"], collision: "≠ AI Model genérico — modelo especializado em prever distribuição de probabilidade sobre sequências de texto" }),
            task({ order: 20, title: "Large Language Model (LLM)", requires: ["Language Model (LM)"], note: "LM em escala — 'large' é qualificador de escala sobre LM" }),
            task({ order: 30, title: "Token", note: "a unidade que um LM processa — autocontido, não exige LM pré-ensinado", revisit: ["Language Models / Language Model (LM)"], resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 40, title: "Tokenization", requires: ["Token"], subtopics: ["Vocabulary (conjunto fixo de tokens possíveis)"], note: "processo que produz tokens" }),
            task({ order: 50, title: "Attention", subtopics: ["Self-Attention (forma usada no Transformer)"], note: "mecanismo de ponderar relevância entre tokens — mecanismo geral de deep learning, não exclusivo de LM", revisit: ["Language Models / Language Model (LM)"] }),
            task({ order: 60, title: "Transformer", requires: ["Attention"], note: "arquitetura construída sobre self-attention" }),
            task({ order: 70, title: "Context Window", requires: ["Token"], note: "nº máximo de tokens que cabem numa chamada", revisit: ["Context Engineering / Context Budget"], resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 80, title: "Next-Token Prediction", requires: ["Language Model (LM)"], note: "o objetivo de treino central de um LM" }),
            task({ order: 90, title: "Autoregressive Generation", requires: ["Next-Token Prediction"], note: "aplicar next-token prediction repetidamente, realimentando a saída" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Deterministic vs Stochastic Output", requires: ["Language Models / Next-Token Prediction"], note: "framing — sampling introduz aleatoriedade na escolha do próximo token" }),
            task({ order: 20, title: "Temperature", requires: ["Deterministic vs Stochastic Output"], note: "controla o quanto a distribuição é 'achatada'" }),
            task({ order: 30, title: "Top-P / Top-K Sampling", requires: ["Deterministic vs Stochastic Output"], subtopics: ["Top-P (nucleus)", "Top-K"], note: "consolidação 2→1 — 2 estratégias de truncar o pool de candidatos" }),
            task({ order: 40, title: "Stopping Conditions (Max Output Tokens / Stop Sequence)", subtopics: ["Max Output Tokens", "Stop Sequence"], note: "consolidação 2→1 — quando parar de gerar" }),
            task({ order: 50, title: "Streaming", note: "como a saída chega", revisit: ["Platform / Web Fundamentals / Server-Sent Events (SSE)"], resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 60, title: "Seed", requires: ["Deterministic vs Stochastic Output"], note: "reprodutibilidade da amostragem" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Prompt", note: "raiz" }),
            task({ order: 20, title: "System Prompt vs User Prompt", requires: ["Prompt"], note: "consolidação 2→1 — contraste direto" }),
            task({ order: 30, title: "Instruction Hierarchy", requires: ["System Prompt vs User Prompt"], note: "qual dos dois prevalece em conflito" }),
            task({ order: 40, title: "Zero-Shot vs Few-Shot Prompting", requires: ["Prompt"], note: "consolidação 2→1" }),
            task({ order: 50, title: "In-Context Learning", note: "capacidade de adaptar comportamento a partir de exemplos/instruções no prompt, sem atualizar pesos — Zero/Few-Shot são aplicações de ICL, não prerequisite", revisit: ["Prompt Engineering / Zero-Shot vs Few-Shot Prompting"] }),
            task({ order: 60, title: "Prompt Template", requires: ["Prompt"], note: "estrutura reutilizável e parametrizada" }),
            task({ order: 70, title: "Structured Prompting", isNew: true, requires: ["Prompt"], note: "técnica de construção/organização do prompt — pedir formato via instrução (soft)", collision: "≠ Schema-Constrained Generation (Structured Generation) — soft (instrução) × hard (decodificação restrita)" }),
            task({ order: 80, title: "Prompt Chaining", requires: ["Prompt"], collision: "≠ Sequential Workflow (Agent Orchestration) — encadear chamadas de LLM sem necessariamente ser agentic" }),
            task({ order: 90, title: "Prompt Versioning", requires: ["Prompt"], note: "canônico aqui — Production AI (Story 23) cita por pointer, não duplica", revisit: ["Production AI / Model Versioning"] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Context", note: "raiz", resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 20, title: "Context Engineering", requires: ["Context"], note: "guarda-chuva — a disciplina de gerenciar o context" }),
            task({ order: 30, title: "Context Assembly", requires: ["Context"], subtopics: ["Context Selection (o que entra)"], note: "montar não exige a moldura da 'disciplina', só o objeto" }),
            task({ order: 40, title: "Context Ordering", requires: ["Context"], note: "como organizar o que foi montado" }),
            task({ order: 50, title: "Context Compression", requires: ["Context"], note: "reduzir volume mantendo sinal" }),
            task({ order: 60, title: "Context Budget", requires: ["Language Models / Context Window"], note: "budget definido em termos do limite físico da janela" }),
            task({ order: 70, title: "Context Overflow", requires: ["Context Budget"], note: "o que acontece ao exceder o budget" }),
            task({ order: 80, title: "Context Rot", requires: ["Context"], subtopics: ["Long Context (o cenário)"], note: "degradação de qualidade com contexto longo, mesmo sem estourar o limite", revisit: ["Language Models / Context Window"] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Structured Output", subtopics: ["JSON Output (formato-exemplo)"], note: "raiz" }),
            task({ order: 20, title: "Schema-Constrained Generation", requires: ["Structured Output"], collision: "≠ Structured Prompting (Prompt Engineering) — mecanismo 'hard' (decodificação restrita por gramática/schema) × 'soft' (instrução)" }),
            task({ order: 30, title: "Output Parsing", requires: ["Structured Output"], note: "extrair dado estruturado do texto gerado" }),
            task({ order: 40, title: "Output Validation", requires: ["Structured Output"], note: "checar se o resultado obedece ao contrato — validar não exige parsing formal como prerequisite" }),
            task({ order: 50, title: "Retry on Invalid Output", requires: ["Output Validation"], note: "reprompt com o erro, não reenvio de request de rede", revisit: ["Architecture / Resilience Patterns / Retry"], resources: [{ type: "deck", deckId: "harness" }] }),
          ],
        }),
        story({
          slug: "embeddings",
          ...
          tasks: [
            task({ order: 10, title: "Vector", subtopics: ["Vector Dimension"], collision: "≠ 'vector'/array em algumas linguagens (Programming Foundations) — conceito matemático × estrutura de dados", note: "raiz — conceito matemático geral" }),
            task({ order: 20, title: "Embedding", requires: ["Vector"], note: "um vetor que representa semântica de um dado (texto, imagem, etc.)" }),
            task({ order: 30, title: "Embedding Model", requires: ["Embedding"], note: "o modelo que produz embeddings" }),
            task({ order: 40, title: "Embedding Space", requires: ["Embedding"], note: "o espaço multidimensional onde embeddings vivem" }),
            task({ order: 50, title: "Semantic Similarity", requires: ["Embedding Space"], note: "proximidade no espaço ≈ similaridade semântica", revisit: ["AI Evaluation / Evaluation Methods (Human / LLM-as-a-Judge / Pairwise / Exact Match / Semantic Similarity)"] }),
            task({ order: 60, title: "Similarity & Distance Metrics (Cosine / Dot Product / Euclidean)", requires: ["Semantic Similarity"], subtopics: ["Cosine Similarity", "Dot Product", "Euclidean Distance"], note: "consolidação 3→1 — as 3 métricas que operacionalizam Semantic Similarity" }),
          ],
        }),
        story({
          slug: "vector-search",
          ...
          tasks: [
            task({ order: 10, title: "Vector Search", requires: ["Embeddings / Embedding Space"], subtopics: ["Similarity Threshold", "Top-K Retrieval"], note: "raiz" }),
            task({ order: 20, title: "Nearest Neighbor Search (k-NN)", requires: ["Vector Search"], note: "consolidação 2→1 — k-NN é NN search parametrizada por k" }),
            task({ order: 30, title: "Approximate Nearest Neighbor (ANN)", requires: ["Nearest Neighbor Search (k-NN)"], note: "alternativa aproximada/escalável ao k-NN exato — contraste direto" }),
            task({ order: 40, title: "Vector Index", requires: ["Approximate Nearest Neighbor (ANN)"], note: "a estrutura de dados que viabiliza ANN em escala" }),
            task({ order: 50, title: "HNSW", requires: ["Vector Index"], note: "algoritmo de índice específico — instância do conceito geral", revisit: ["Programming Foundations / Data Structures / Graph"] }),
            task({ order: 60, title: "Vector Database", requires: ["Vector Index"], note: "o produto/categoria que empacota índice + storage + query. Pinecone/Weaviate/pgvector = exemplos, não Tasks" }),
          ],
        }),
        story({
          slug: "chunking",
          ...
          tasks: [
            task({ order: 10, title: "Chunking", subtopics: ["Chunk Size", "Chunk Overlap"], note: "raiz — dividir documentos em pedaços indexáveis" }),
            task({ order: 20, title: "Chunking Strategies (Fixed-Size / Recursive / Semantic / Document Structure)", requires: ["Chunking"], subtopics: ["Fixed-Size", "Recursive", "Semantic", "Document Structure"], note: "consolidação 4→1 — o catálogo de estratégias" }),
          ],
        }),
        story({
          slug: "retrieval",
          ...
          tasks: [
            task({ order: 10, title: "Retrieval", collision: "≠ RAG (Story 11, arquitetura que usa Retrieval) ≠ Grounding (Story 11, a propriedade que RAG busca)", note: "raiz — a operação geral de IR (buscar itens relevantes dado um query)" }),
            task({ order: 20, title: "Semantic Search", subtopics: ["Dense Retrieval (alt-nome acadêmico de IR)"], note: "buscar por proximidade de significado — Vector Search é uma implementação possível, não prerequisite da definição", revisit: ["Vector Search / Vector Search"] }),
            task({ order: 30, title: "Keyword Search", subtopics: ["Sparse Retrieval (alt-nome acadêmico de IR)"], collision: "contraste com Semantic Search", note: "busca lexical — autocontida (tipo grep)" }),
            task({ order: 40, title: "BM25", requires: ["Keyword Search"], note: "algoritmo de ranking lexical específico" }),
            task({ order: 50, title: "Hybrid Search", requires: ["Semantic Search", "Keyword Search"], note: "combina os dois — síntese genuína" }),
            task({ order: 60, title: "Metadata Filtering", note: "filtrar por metadado estruturado — autocontido (tipo WHERE clause)", revisit: ["Retrieval / Retrieval"] }),
            task({ order: 70, title: "Reranking", requires: ["Retrieval"], subtopics: ["Cross-Encoder Reranking", "nota: 'candidate set' (Candidate Retrieval, absorvido)"], note: "2º estágio de refino — aplica-se a qualquer 1º estágio, não só híbrido" }),
          ],
        }),
        story({
          slug: "rag",
          ...
          tasks: [
            task({ order: 10, title: "Retrieval-Augmented Generation (RAG)", requires: ["Retrieval / Retrieval"], collision: "≠ Retrieval (mecanismo geral) — RAG é a arquitetura que USA retrieval para aumentar geração" }),
            task({ order: 20, title: "RAG Pipeline (Indexing / Retrieval / Generation)", requires: ["Retrieval-Augmented Generation (RAG)"], subtopics: ["Indexing", "Retrieval", "Generation"], note: "consolidação 4→1" }),
            task({ order: 30, title: "Query Transformation", requires: ["Retrieval / Retrieval"], subtopics: ["Query Expansion", "Multi-Query Retrieval"], note: "consolidação 3→1 — técnica de IR geral, não exclusiva de RAG" }),
            task({ order: 40, title: "Grounding", collision: "≠ Retrieval ≠ RAG — a propriedade que RAG busca (saída verificavelmente ligada às fontes); conceito mais geral, aplica a tool outputs e doc fixo. Sem revisitOf/sinonímia com Faithfulness (Story 19)", note: "RAG é só UMA via de alcançar grounding", revisit: ["RAG / Retrieval-Augmented Generation (RAG)"] }),
            task({ order: 50, title: "Source Attribution", requires: ["Grounding"], note: "citar fontes — mecanismo concreto de demonstrar grounding" }),
            task({ order: 60, title: "Retrieval Failure", requires: ["Retrieval-Augmented Generation (RAG)"], note: "falha semântica (retrieval irrelevante), distinta de falha de transporte (Architecture/Resilience)" }),
            task({ order: 70, title: "RAG Evaluation", requires: ["Retrieval-Augmented Generation (RAG)"], note: "framing — métricas concretas vivem em AI Evaluation (Story 19)", revisit: ["AI Evaluation / Faithfulness & Answer Relevance", "AI Evaluation / Retrieval Precision & Recall"] }),
          ],
        }),
        story({
          slug: "tool-calling",
          ...
          tasks: [
            task({ order: 10, title: "Tool", note: "raiz — capacidade que um sistema de IA pode invocar; não agent-specific" }),
            task({ order: 20, title: "Tool Calling", requires: ["Tool"], subtopics: ["Function Calling (termo da OpenAI para o mesmo conceito)"], note: "a Requires de Story Tool Calling → Structured Generation é preservada; sem Task-edge artificial", revisit: ["Structured Generation / Structured Output"] }),
            task({ order: 30, title: "Tool Schema", requires: ["Tool"], subtopics: ["Tool Arguments"], note: "schema é insumo de Tool Calling, não o contrário" }),
            task({ order: 40, title: "Tool Selection", requires: ["Tool Schema"], note: "escolher entre tools dado seus schemas" }),
            task({ order: 50, title: "Tool Result", requires: ["Tool Calling"], note: "o retorno de executar a tool, realimentado ao modelo" }),
            task({ order: 60, title: "Tool Error Handling", requires: ["Tool Result"], subtopics: ["Tool Retry"], revisit: ["Architecture / Resilience Patterns"], note: "mecanismo distinto, não reenvio de rede genérico" }),
            task({ order: 70, title: "Tool Permissions", requires: ["Tool"], note: "escopo de autorização por tool", revisit: ["Platform / Authorization", "AI Safety & Guardrails / Least Privilege for Tools"] }),
            task({ order: 80, title: "Tool Execution Loop", requires: ["Tool Result", "Tool Error Handling"], collision: "≠ Agent Loop (Story 14) — este é o loop mecânico de UMA chamada de tool" }),
          ],
        }),
        story({
          slug: "mcp",
          ...
          tasks: [
            task({ order: 10, title: "Model Context Protocol (MCP)", collision: "'Context' no nome ≠ Context Engineering (Story 05) — MCP usa 'context' num sentido mais amplo (tools/resources/prompts expostos)", note: "protocolo padrão (não produto/vendor); root no Task DAG; a Requires de Story MCP → Tool Calling é preservada", revisit: ["Tool Calling / Tool Schema"] }),
            task({ order: 20, title: "MCP Architecture (Host / Client / Server)", requires: ["Model Context Protocol (MCP)"], subtopics: ["Host", "Client", "Server"], note: "consolidação 3→1 — os 3 papéis arquiteturais" }),
            task({ order: 30, title: "MCP Primitives (Tools / Resources / Prompts)", requires: ["Model Context Protocol (MCP)"], subtopics: ["Tools", "Resources", "Prompts"], note: "consolidação 3→1 — primitivas não exigem os 3 papéis, só o protocolo" }),
            task({ order: 40, title: "MCP Transport", requires: ["MCP Architecture (Host / Client / Server)"], subtopics: ["stdio", "HTTP/SSE"], note: "como Host/Client/Server se comunicam" }),
            task({ order: 50, title: "MCP vs API", requires: ["Model Context Protocol (MCP)", "Platform / API Fundamentals / API"], note: "comparação — desambigua confusão comum" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Deterministic vs Agentic Workflow", note: "framing — a fronteira central desta Story; sem prerequisite conceitual real (root)", revisit: ["Tool Calling / Tool Execution Loop"] }),
            task({ order: 20, title: "AI Agent", note: "raiz do conceito — Agent é definível diretamente, a moldura determinístico×agentic é pedagógica", revisit: ["Agent Fundamentals / Deterministic vs Agentic Workflow"] }),
            task({ order: 30, title: "Agent vs LLM", requires: ["AI Agent"], note: "agente = LLM + tools + loop + autonomia, não o modelo cru", revisit: ["AI Fundamentals / AI System"] }),
            task({ order: 40, title: "Agentic System", isNew: true, requires: ["AI Fundamentals / AI System", "AI Agent"], collision: "AI System ≠ Agentic System ≠ Agentic Workflow — especialização de AI System composta de agentes" }),
            task({ order: 50, title: "Agentic Workflow", requires: ["Agentic System"], note: "fluxo executado por/dentro do sistema agentic", resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 60, title: "Agent Loop", requires: ["Agentic Workflow"], subtopics: ["Planning", "Acting", "Observation", "Reflection"], collision: "≠ Tool Execution Loop (Story 12) — ciclo plan-act-observe-reflect mais amplo", note: "consolidação 5→1", revisit: ["Tool Calling / Tool Execution Loop"] }),
            task({ order: 70, title: "ReAct", requires: ["Agent Loop"], note: "padrão nomeado específico — instância do Agent Loop" }),
            task({ order: 80, title: "Human-in-the-Loop (HITL)", note: "mecanismo de controle inserido no loop; sem prerequisite conceitual real (root)", revisit: ["Agent Fundamentals / Agent Loop", "AI Safety & Guardrails / Human Approval"] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Orchestration (Workflow)", note: "raiz — 'Orchestration' e 'Workflow' do rascunho fundidos; explicável com vocabulário genérico de 'passos'", revisit: ["Agent Fundamentals / Agent Loop"] }),
            task({ order: 20, title: "Router", requires: ["Orchestration (Workflow)"], note: "direciona execução para caminhos/agentes diferentes" }),
            task({ order: 30, title: "Workflow Composition (Sequential / Parallel / Conditional)", requires: ["Orchestration (Workflow)"], subtopics: ["Sequential", "Parallel", "Conditional"], note: "consolidação 3→1 — os 3 padrões de composição" }),
            task({ order: 40, title: "Agent State", note: "rastrear estado ao longo do workflow — autocontido (root)", revisit: ["Agent Orchestration / Orchestration (Workflow)"] }),
            task({ order: 50, title: "Agent Termination", requires: ["Agent State"], note: "quando parar — depende de avaliar o estado" }),
          ],
        }),
        story({
          slug: "ai-memory",
          ...
          tasks: [
            task({ order: 10, title: "Agent Memory", requires: ["Agent Fundamentals / AI Agent"], note: "raiz" }),
            task({ order: 20, title: "Memory vs Context", requires: ["Agent Memory", "Context Engineering / Context"], collision: "memory é persistida entre sessões/turnos; context é o que está na janela atual" }),
            task({ order: 30, title: "Short-Term vs Long-Term Memory", requires: ["Agent Memory"], note: "consolidação 2→1 — distinção autocontida" }),
            task({ order: 40, title: "Memory Types (Conversation / Semantic / Episodic)", requires: ["Short-Term vs Long-Term Memory"], subtopics: ["Conversation", "Semantic", "Episodic"], note: "consolidação 3→1 — tipos de memória de longo prazo por conteúdo" }),
            task({ order: 50, title: "Memory Retrieval", requires: ["Agent Memory"], note: "recuperar não exige a taxonomia completa de tipos", revisit: ["Vector Search / Vector Search", "Retrieval / Retrieval"] }),
            task({ order: 60, title: "Memory Consolidation", requires: ["Agent Memory"], note: "resumir/compactar memória ao longo do tempo — operação irmã de retrieval, não dependente" }),
          ],
        }),
        story({
          ...
          summary:
            ...
            "→ trade-offs (capstone).",
          tasks: [
            task({ order: 10, title: "Multi-Agent System", requires: ["Agent Orchestration / Orchestration (Workflow)"], note: "raiz" }),
            task({ order: 20, title: "Agent Coordination", requires: ["Multi-Agent System"], note: "o problema geral — framing antes dos mecanismos" }),
            task({ order: 30, title: "Agent Role", requires: ["Multi-Agent System"], note: "o que cada agente se especializa em" }),
            task({ order: 40, title: "Agent Communication", requires: ["Multi-Agent System"], note: "como agentes trocam informação" }),
            task({ order: 50, title: "Agent Delegation", requires: ["Agent Communication"], note: "delegar trabalho exige canal de comunicação" }),
            task({ order: 60, title: "Agent Handoff", requires: ["Agent Delegation"], note: "lar canônico único (absorve a 'Handoff' removida da Story 15) — delegação com transferência completa de controle" }),
            task({ order: 70, title: "Supervisor Pattern", requires: ["Agent Delegation"], note: "padrão nomeado — um agente supervisiona/delega para outros" }),
            task({ order: 80, title: "Multi-Agent Trade-offs", requires: ["Agent Coordination"], note: "capstone — quando multi-agente vale a complexidade; pesa o problema geral, não um padrão específico" }),
          ],
        }),
        story({
          slug: "model-routing",
          ...
          tasks: [
            task({ order: 10, title: "Model Router", requires: ["Model Inference"], note: "raiz" }),
            task({ order: 20, title: "Static vs Dynamic Routing", requires: ["Model Router"], subtopics: ["Static Routing", "Dynamic Routing"], note: "consolidação 2→1" }),
            task({ order: 30, title: "Routing Criteria (Capability / Cost / Latency)", requires: ["Model Router"], subtopics: ["Capability", "Cost", "Latency"], note: "consolidação 3→1 — critério (o quê) independe do modo (como decide)", revisit: ["Platform / Performance Engineering / Latency"] }),
            task({ order: 40, title: "Model Fallback", requires: ["Model Router"], note: "canônico aqui — Production AI (Story 23) revisita (revisitOf intra-Epic)", revisit: ["Production AI / Model Fallback", "Architecture / Resilience Patterns"] }),
            task({ order: 50, title: "Multi-Provider Architecture", note: "suportar múltiplos provedores é pré-condição do fallback entre eles (root). OpenAI/Anthropic/Gemini/Bedrock/Vertex AI = exemplos, não Tasks", revisit: ["Model Routing / Model Fallback", "Model Routing / Model Router"] }),
          ],
        }),
        story({
          slug: "ai-evaluation",
          ...
          tasks: [
            task({ order: 10, title: "Evaluation (Evals)", note: "raiz — disciplina paralela a Testing, versão específica de IA", revisit: ["Testing & Quality Engineering / Testing Fundamentals"] }),
            task({ order: 20, title: "Evaluation Dataset", requires: ["Evaluation (Evals)"], subtopics: ["Golden Dataset (versão curada/rotulada)"], note: "dataset PARA avaliação" }),
            task({ order: 30, title: "Offline vs Online Evaluation", requires: ["Evaluation (Evals)"], note: "consolidação 2→1 — online eval frequentemente nem usa dataset fixo", revisit: ["Testing & Quality Engineering / Testing Strategy"] }),
            task({ order: 40, title: "Evaluation Methods (Human / LLM-as-a-Judge / Pairwise / Exact Match / Semantic Similarity)", requires: ["Evaluation (Evals)"], subtopics: ["Human", "LLM-as-a-Judge", "Pairwise", "Exact Match", "Semantic Similarity"], collision: "'Semantic Similarity' aqui = aplicação de Embeddings / Semantic Similarity (Story 07) como método de avaliação", note: "consolidação 5→1 — métodos independem do momento (offline/online)" }),
            task({ order: 50, title: "Retrieval Precision & Recall", requires: ["Evaluation Dataset"], note: "consolidação 2→1 — métricas clássicas de IR, autocontidas dado um dataset", revisit: ["RAG / RAG Evaluation"] }),
            task({ order: 60, title: "Faithfulness & Answer Relevance", requires: ["Evaluation (Evals)"], note: "consolidação 2→1 — mede qualidade de GERAÇÃO; sem revisitOf/sinonímia com Grounding (PENDENTE-2)", revisit: ["RAG / Grounding"] }),
            task({ order: 70, title: "Regression Evaluation", requires: ["Evaluation Methods (Human / LLM-as-a-Judge / Pairwise / Exact Match / Semantic Similarity)"], note: "regressão exige um método de score para comparar", revisit: ["Testing & Quality Engineering / Testing Strategy / Regression Testing"] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Guardrails", note: "raiz", resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 20, title: "Input vs Output Guardrails", requires: ["Guardrails"], subtopics: ["Input Guardrails", "Output Guardrails"], note: "consolidação 2→1", resources: [{ type: "deck", deckId: "harness" }] }),
            task({ order: 30, title: "Hallucination", note: "a falha de grounding — autoexplicável sem o conceito de Grounding (root)", revisit: ["RAG / Grounding"] }),
            task({ order: 40, title: "Prompt Injection", requires: ["Guardrails"], note: "entrada não confiável tratada como instrução em contexto privilegiado", revisit: ["Platform / Application Security"] }),
            task({ order: 50, title: "Indirect Prompt Injection", requires: ["Prompt Injection"], note: "injeção via conteúdo recuperado/de tool, não input direto", revisit: ["RAG"] }),
            task({ order: 60, title: "Jailbreak", requires: ["Guardrails"], collision: "≠ Prompt Injection — contornar o alinhamento do próprio modelo, não injetar instrução via conteúdo externo" }),
            task({ order: 70, title: "Data Leakage", requires: ["Guardrails"], note: "vazamento de dado de treino/contexto sensível na saída" }),
            task({ order: 80, title: "PII Handling", note: "tipo específico de vazamento — pode ser tratado sem Data Leakage pré-ensinado (root)", revisit: ["AI Safety & Guardrails / Data Leakage", "Platform / Application Security"] }),
            task({ order: 90, title: "Tool Abuse", requires: ["Tool Calling / Tool Permissions"], note: "agente manipulado a usar mal suas tools — pode ocorrer sem jailbreak", revisit: ["AI Safety & Guardrails / Jailbreak"] }),
            task({ order: 100, title: "Least Privilege for Tools", requires: ["Tool Calling / Tool Permissions"], canonical: false, revisitOf: "Platform / Authorization / Principle of Least Privilege", note: "mesmo padrão de Cloud Security / Least Privilege in Cloud (Epic 05); a revisitOf já é a estrutura correta" }),
            task({ order: 110, title: "Human Approval", requires: ["Agent Fundamentals / Human-in-the-Loop (HITL)"], note: "revisita HITL com framing de segurança — pausa e exige aprovação explícita antes de ação de risco; defesas complementares/independentes de Least Privilege", resources: [{ type: "deck", deckId: "harness" }] }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "Fine-Tuning", requires: ["AI Fundamentals / Training"], note: "raiz — forma/continuação de Training" }),
            task({ order: 20, title: "Supervised Fine-Tuning (SFT) / Instruction Tuning", requires: ["Fine-Tuning"], note: "consolidação 2→1 — Instruction Tuning é SFT sobre dados de seguir-instrução" }),
            task({ order: 30, title: "PEFT (LoRA)", requires: ["Fine-Tuning"], subtopics: ["LoRA"], note: "consolidação 2→1 — eficiência é técnica geral sobre fine-tuning, não exclusiva de SFT" }),
            task({ order: 40, title: "Distillation", note: "treinar modelo menor para imitar um maior — categoria de técnica diferente, não subtipo de fine-tuning (root)", revisit: ["Model Adaptation / Fine-Tuning"] }),
            task({ order: 50, title: "Quantization", collision: "técnica de compressão/eficiência, ortogonal a fine-tuning (não altera comportamento, reduz precisão numérica)", note: "root" }),
            task({ order: 60, title: "RAG vs Fine-Tuning", requires: ["RAG / Retrieval-Augmented Generation (RAG)", "Fine-Tuning"], note: "capstone comparativo" }),
          ],
        }),
        story({
          ...
          tasks: [
            task({ order: 10, title: "LLM Tracing", requires: ["Platform / Observability / Distributed Tracing (Span / trace context)"], note: "distributed tracing aplicado a cadeias de chamada de LLM" }),
            task({ order: 20, title: "Prompt Logging", requires: ["Platform / Observability / Structured Logging"], note: "structured logging aplicado a prompts — técnica paralela ao tracing, não dependente" }),
            task({ order: 30, title: "Token Usage", note: "métrica sem equivalente em Platform — contar tokens não exige o mecanismo de tracing (root)", revisit: ["Language Models / Token"] }),
            task({ order: 40, title: "Cost Tracking", requires: ["Token Usage"], note: "custo = tokens × preço" }),
            task({ order: 50, title: "Latency Tracking", requires: ["Platform / Performance Engineering / Latency"], note: "canônico lá; aqui é o ponto de medição específico de IA" }),
            task({ order: 60, title: "Tool & Retrieval Tracing", requires: ["LLM Tracing"], note: "consolidação 2→1 — 2 especializações do tracing para operações de IA" }),
            task({ order: 70, title: "Failure Analysis", note: "capstone — sinais concretos para diagnosticar; sem prerequisite conceitual (root)", revisit: ["AI Observability / Tool & Retrieval Tracing", "AI Observability / Prompt Logging", "Testing & Quality Engineering / Debugging / Root Cause Analysis", "Platform / Reliability Engineering / Postmortem (Blameless)"] }),
          ],
        }),
        story({
          slug: "production-ai",
          ...
          tasks: [
            task({ order: 10, title: "AI System Reliability", requires: ["AI Fundamentals / AI System"], note: "raiz/framing — propriedade DO sistema" }),
            task({ order: 20, title: "Model Latency", requires: ["Platform / Performance Engineering / Latency"], note: "profundidade nova (batching, tamanho de modelo) — explicável sem a moldura de 'reliability'" }),
            task({ order: 30, title: "Time to First Token (TTFT)", requires: ["Model Latency"], note: "métrica específica de streaming, sem equivalente em Platform", revisit: ["Model Inference / Streaming"] }),
            task({ order: 40, title: "Token Throughput", requires: ["Platform / Performance Engineering / Throughput"], note: "família de métrica diferente (taxa, não atraso); AI Observability / Token Usage fica pointer/contexto", revisit: ["AI Observability / Token Usage"] }),
            task({ order: 50, title: "Model Cost", requires: ["AI Observability / Cost Tracking"], note: "gestão de custo em produção, constrói sobre o tracking já estabelecido" }),
            task({ order: 60, title: "Token Budget", requires: ["Model Cost"], collision: "≠ Context Budget (Story 05) — limite ECONÔMICO (quanto se está disposto a gastar) × limite de CAPACIDADE (o que cabe na janela)" }),
            task({ order: 70, title: "Caching LLM Responses (Semantic Cache)", requires: ["Platform / Caching / Cache"], canonical: false, revisitOf: "Platform / Caching / Cache", note: "cache por similaridade semântica da query, não match exato; consolida com o alvo estrutural do revisitOf" }),
            task({ order: 80, title: "Rate Limiting", canonical: false, revisitOf: "Platform / API Fundamentals / Rate Limiting", note: "aplicação: controle de custo/quota de chamadas de LLM (root)" }),
            task({ order: 90, title: "Model Fallback", canonical: false, revisitOf: "Model Routing / Model Fallback", note: "revisita intra-Epic (mesmo padrão de GraphQL / N+1 in Resolvers no Epic 05) (root)" }),
            task({ order: 100, title: "Retry Strategy", canonical: false, revisitOf: "Architecture / Resilience Patterns / Retry", note: "aplicação em produção de IA (root)" }),
            task({ order: 110, title: "Model Versioning", note: "canônico — sem lar anterior. Prompt Versioning NÃO ganha Task própria aqui — já canônico em Prompt Engineering (Story 04), citada só como pointer (root)", revisit: ["Prompt Engineering / Prompt Versioning"] }),
            task({ order: 120, title: "Evaluation in CI/CD", requires: ["AI Evaluation / Regression Evaluation"], note: "gate de deploy por qualidade de IA", revisit: ["Platform / CI-CD / CI/CD Pipeline"] }),
            task({ order: 130, title: "AI System Monitoring", requires: ["AI Observability / Failure Analysis"], note: "capstone final — pré-deploy (gate) × pós-deploy (monitoramento) são preocupações paralelas", revisit: ["Platform / Reliability Engineering"] }),
          ],
        }),
      ],
    }),
